...
- `completedChapters`: `string[]` of grammar chapter ids whose end-of-chapter review step has already been shown (or, for a chapter with no anchored contrast lessons, silently acknowledged) - same additive pattern as `grammarQueue` (default-to-`[]` at hydration, no version gate). The CURRENT chapter (what to introduce next) is always derived from the teaching order, never stored - this field only remembers which chapters' one-time step has already fired. See Grammar Activity → Chapter-driven introduction below.
...
- `variantOf?` / `variantRelation?`: set when this point is a **realization variant** of another point (`variantOf` is the canonical's id; `variantRelation` is `'contraction'`/`'politeness'`/`'particle'`/`'particle+politeness'`). A realization variant is the SAME construction written differently (じゃ and それじゃ are contractions of それでは); it is **not** independently taught - only the canonical is in the teaching order, and its quiz card rotates through the realizations (`applyVariantRotation` in `grammarSelectors.ts`, graded interchangeably by register) against the canonical's single SRS entry, so mastery is one climb rather than N. Authored in `gokan-dataset`'s `data/raw/grammar/variants.json`, compiled into `index/variant-groups.json` (`GrammarVariantGroupIndex = Record<canonicalId, { id, relation, formalityLevel?, title }[]>`, includes the canonical with relation `'canonical'`). **A realization variant never ALSO carries a `family`**: the canonical carries the family, so a variant is not an independent family member and never appears in a sibling's `relatedPoints`. `build-grammar.ts` throws if `formality.json` gives a variant both (the fix for the じゃ-vs-それじゃ inconsistency, where じゃ (n5-004) had wrongly stayed a `sequence-then` member while それじゃ (n5-005) did not). Surfaced on `GrammarDetailScreen` by `GrammarVariantsCard`.

> [!NOTE]
> `formalityLevel`/`usageNote`/`family` are populated for a growing, ongoing subset of points via `gokan-dataset`'s `data/raw/grammar/formality.json` (see its `GRAMMAR_FORMALITY_TRACKING.md` for current coverage). All three fields are optional and inert wherever a point has none. `compiled/grammar/index/families.json` (`Record<familyId, { name, memberIds }>`, mirroring `index/jlpt.json`'s shape) is also compiled but not directly consumed anywhere in the app yet - reserved for a possible future family-browsing view.
>
> **Contrast lessons (issue #62)**: `compiled/grammar/index/contrasts.json` (`Record<familyId, { name, lessons: { id, title, points, cases: { focus, vs[], situation, guidance }[], taughtInChapterId? }[], interchangeable? }>`) carries authored *situational* disambiguation between confusable family members - the "which one do I reach for, and why not the obvious alternative" that a per-point `usageNote` cannot answer (から and ので both mean "because", but only ので fits an apology). Three distinct concerns: a **lesson** is a confusability-first sub-grouping of a family (target 5-6 points, soft cap; a small family can be one lesson, a large one is split); a **case** is one concrete situation inside a lesson, and may only name points that lesson covers; and **interchangeability** is a separate grading rule (below). A lesson is NOT a chapter - a chapter is a slot in the introduction order, a lesson is a set of points learners mix up, and they cut across each other by design (a lesson was called a "chunk" until the collision proved confusing in practice). The dataset's `docs/GRAMMAR_TEACHING_MODEL.md` explains the whole model in plain English and `docs/CURRICULUM.md` is the generated inventory of every chapter and lesson. Authored in the dataset's `data/raw/grammar/contrasts.json` (AI-drafted, human-reviewed), compiled + strictly validated by `build-grammar.ts`'s pure `compileContrasts` (every `focus`/`vs`/`points` id must be a genuine, non-dropped member of the family it claims, never a `variant`-axis point, and each case's `focus`/`vs` must be among its own lesson's `points`). Each case is **directed** - `focus` is the point met LATER in the teaching order. Consumed via `GrammarService.loadContrasts`/`loadContrastsByFocus`. Surfaced by `GrammarContrastCard` at the `focus` point's introduction, but only once every `vs` sibling is already known (the deferral rule is `utils/grammarContrast.utils.ts`'s pure `selectReadyContrasts`), and revisitably on `GrammarFamilyScreen` (route `/grammar/family/:familyId`, linked from `GrammarRelatedPointsCard`'s "Compare when to use each"). Coverage is now the full set the curriculum calls for: **66 families, 96 lessons, 140 cases**, one per chapter-cluster with a real distinction to teach (it was 4 families / 6 cases at the starter batch). lessons are scoped to the confusable set rather than to the family wherever the family grouping was too coarse - `sequence-then` is split into a そして/それから lesson and a じゃ/それでは lesson, because those mean different things and a four-way lesson over them would be incoherent.
>
> Two fields come from the curriculum rather than the authored file. **`lesson.taughtInChapterId`** is stamped by `build-curriculum.ts` (the only build step that knows the chapters): the chapter of whichever point it covers the teaching order introduces last, i.e. the earliest moment at which every point the lesson names is a real memory. The app does not gate the introduction-time card on it - `selectReadyContrasts`'s "every `vs` sibling known" check is the per-learner runtime equivalent - but it IS what the end-of-chapter review step keys off (see Grammar Activity → Chapter-driven introduction below: `selectChapterEndFocusIds` collects every lesson anchored to a completed chapter). The curriculum issue assumed the stronger rule that a lesson may not span chapters **at all**; that rule would delete the から/ので lesson issue #62 opens with (から is introduced in `n5-c16`, ので in `n4-c12`), so what the build enforces instead is the invariant that carries the weight: **a case's `focus` may never be introduced before one of its `vs` siblings** (a hard error). 3 of 96 lessons span chapters, and the build counts them.
>
> **`family.interchangeable`** is the `axis: 'variant'` members, when a family has two or more. Those can never carry a lesson (a build error if one tries), but staying silent about them is worse than a line of text: a learner meeting the fourth of ten near-identical "whether A or B" forms with no comment concludes a distinction exists and goes hunting for one. `GrammarService.loadInterchangeableByPoint()` keys them by point id; `GrammarContrastCard` renders the note in place of a lesson, deferred by `selectKnownInterchangeableSiblings` until at least one sibling is actually known, and `GrammarFamilyScreen` renders an "Interchangeable" card. A family can carry this and `lessons: []`, so an entry with no lessons is valid rather than missing.
>
> **Interchangeability grading (issue #62)**: a family sibling typed into a cloze pattern blank in place of the expected marker is graded by *proximity* rather than flat-wrong, gated by a per-point **`slot`** (`clause-final` / `sentence-initial` / `predicate-final` / `pre-noun` / `adverbial`, authored in `formality.json`). `applyFamilyInterchange` (`grammarSelectors.ts`) widens a pattern blank with siblings that fill the **same slot** and are `register`/`variant` axis: same formality -> added to `acceptLists` (grades `correct`), different formality -> `acceptListsMinor` (grades `minor_error`), reusing the exact two-tier mechanism the variant-group rotation uses. A different-slot sibling (でも in a clause-final けど blank is ungrammatical) or a `constraint`-axis sibling (changes meaning) is never added, so it stays `wrong`. `slot` is authored only for the starter families' register members so far.
...
- `'session-complete'`: This session cleared everything it committed to, but reviews the per-session cap left out are still due (see Session quiz cap under State Management). Vocab only: `GrammarSessionState` excludes it alongside `'learn-kanji'`.
...
**No same-session collision handling lives in `applyAnswer` anymore.** An earlier version had a correct reading answer stagger a due meaning's `dueDate` +12h so the two weren't asked back to back, but deliberately left production unstaggered (it re-seeds against reading's own cadence, so a fixed push just repeated every session and production was never once asked - reported from staging after the first release: reading and meaning came up daily while production never appeared). That asymmetry, and the stagger's second-order bug (it could resolve a committed meaning task's `SessionStats.done` without the user ever answering it - see `docs/MODIFICATION_LOG.md`), were both symptoms of solving a session-layer problem inside the SRS formula. It's now solved once, at the session layer: `dedupTaskKeysByVocab` (`quizSelectors.ts`) keeps at most one task per vocab per session (reading > meaning > production) when the committed set is built, uniformly across all three types, without mutating any entry's persisted schedule - see Session quiz cap below.
...
### Production quiz synonym-aware grading (`srs.service.ts`, `useQuizOrchestration.ts`, issue #71 Part B)

The production cue is a bare gloss list, which cannot distinguish near-synonyms sharing overlapping glosses (必ず vs 常に both gloss roughly "certainly/always" - the issue's motivating case). Grading every gloss-overlapping answer as `minor_error` would reward that ambiguity rather than fix it, so a wrong production answer is checked against the target word's near-synonym cluster and graded in one of two tiers rather than flat `wrong`.

- **Dataset**: `compiled/index/synonyms.json` (`SynonymIndex = Record<vocabId, {id: string; relation: SynonymRelation}[]>`, `SynonymRelation = 'interchangeable' | 'confusable'`), symmetric, built in `gokan-dataset` from auto-derived same-POS gloss-overlap clustering with a hand-authored relation tier (mirrors how `formality.json` backs the grammar `family`/`usageNote` fields). A word with no entry is inert - graded exactly as if this feature didn't exist. Loaded whole and cached via `VocabularyService.loadSynonymsIndex()`.
- **Resolved at card-load time, not on submit.** `useQuizOrchestration`'s loading effect, for a production card only, looks up the target's cluster and fetches each candidate's own `Vocabulary` (same "resolve before render" pattern `computeBlankPlan` uses for its accept-lists), building `QuizState.currentProductionSynonyms: ProductionSynonymCandidate[]` (`{vocabId, relation, vocab: Pick<Vocabulary, 'reading'|'writtenForm'|'mergedVocabs'>}`) before dispatching `LOAD_VOCAB_SUCCESS`. This is what keeps `submitAnswer`'s grading synchronous - no fetch ever happens on submit. Reset to `[]` on `LOAD_VOCAB_START`, same pattern as `currentProductionCloze`. A candidate id that fails to load (stale index reference) is dropped rather than failing the card.
- **`SRSService.evaluateProductionSynonyms(userInput, candidates)`** - only ever called once the target's own `evaluateProductionAnswer` has graded `wrong`. Reuses `evaluateProductionAnswer` per candidate (the identical accept-list logic used to grade the target itself, so "written forms match exactly, readings stay fuzzy" applies uniformly to every candidate too), returning the first candidate the input matches or `null`.
- **Two tiers, not one**, because crediting every collision equally would reward exactly the confusion the issue opens with (必ず and 常に are precisely *not* interchangeable):
  - `interchangeable`: grades `minor_error` through the **normal** `applyAnswer` path (forcedResult `'minor_error'`) - real credit, since either word genuinely answers the cue. `submitAnswer` builds a message naming the word that was actually being tested (e.g. "きっと is also accepted here - the word being tested was 必ず (かならず).").
  - `confusable`: **no strength change at all**. `SRSService.applyConfusableSynonymAnswer(vocab, now)` bypasses `applyAnswer` entirely and reuses the existing per-quiz-type retry machinery directly - it sets `needsRetry.production = true` and leaves `memoryStrength`/`interval`/`difficulty`/`dueDate` completely untouched (only `lastReviewedAt`/`totalReviews` record that an attempt happened), matching the established invariant that a retry is training only. The card simply re-asks (via the normal `needsRetry.production` actionability check) until the target itself is produced, so there is no way to coast on a confusable answer. A subsequent correct retry attempt then runs through `applyAnswer`'s own pre-existing retry branch, which likewise applies no strength change - consistent with every other quiz type's retry behavior, not a new rule.
- **An unrelated wrong answer, or a word with no synonym-index entry, is unaffected** - `evaluateProductionSynonyms` returns `null` and `result` stays the plain `evaluateProductionAnswer` verdict.
- Surfaced identically on both production cards (gloss-prompt and cloze, issue #72) since both share `submitAnswer`/`continueToNext` - only `VocabBaseQuizCard`'s message-display condition needed widening (it previously only showed `feedback.message` for `minor_error`; a `confusable` collision grades `wrong` by design, so it now also shows the message whenever `feedback.synonymRelation` is set). `VocabProductionClozeQuizCard` already rendered `feedback.message` unconditionally.
...
- **`splitSentenceAtBlank(cloze)`** (pure) - `before`/`blank`/`after` string slices off `sentence.original`; `before + blank + after` always reproduces the sentence exactly. **`splitClozeContext(cloze)`** (pure) is its interactive-rendering counterpart: it returns the two `Sentence` fragments on either side of the blank (each side's `matches` filtered to that side, `after`'s offsets rebased), so the card renders its surrounding context through the shared `InteractiveSentence` (clickable, gloss-on-hover words) instead of flat text. Only the *blanked occurrence* of the target is dropped (other occurrences of the same word remain); a match straddling the blank boundary is dropped defensively.
- **`emphasizeGloss(englishText, glosses)`** (pure) - decides how the English cue signals which word to produce (the bare translation was too ambiguous to tell). Matches the target vocab's own glosses (cleaned of parentheticals and a leading "to"/article, longest/most-specific first, whole-word, ≥3 chars) against the translation: on a hit, returns the span to **bold in place**; on no verbatim hit (conjugated/reworded translations, e.g. "has" for gloss "to have"), returns `labelGlosses` for the card to show as a small gloss label instead. There is no word-level JP↔EN alignment in the dataset, so this is deliberately best-effort with a guaranteed fallback so every card indicates the word one way or the other.
...
- **Deliberate departure from the gloss card's "nothing Japanese before feedback" rule** (see `VocabProductionQuizCard`'s updated doc comment): the surrounding sentence is visible before answering (rendered interactively via `splitClozeContext` + `InteractiveSentence`, see above), since it's the cue that makes the item well-posed. The invariant that still holds on both cards is that the **target word** stays hidden until feedback.
...
- `loadSynonymsIndex()`: Load the near-synonym cluster index for production-quiz grading (vocabId → `{id, relation}[]`, issue #71 Part B - see Production quiz synonym-aware grading). Loaded whole and cached, like `loadFrequencyIndex`/`loadJlptIndex`; missing/unreachable degrades to `null` rather than throwing, since the feature is inert wherever absent.

**Data Location**: `data/compiled/`
- Indexes: `index/kklc.json`, `index/kklc-kanji.json`, `index/frequency.json`, `index/kanji-vocab.json` (kanji character → vocab IDs containing it, frequency-sorted), `index/jlpt.json` (JLPT level → `{id, containedKanji}[]`, frequency-sorted within each level; ~6.4k of the ~36k vocab carry a level), `index/synonyms.json` (vocabId → near-synonym cluster, see `loadSynonymsIndex` above)
...
- `loadTeachingOrder()`: Load the authored introduction order (`index/teaching-order.json`, `{ order, chapters }` - see Grammar Activity → Chapter-driven introduction below). Returns `null` on failure so callers fall back to the plain JLPT walk.
- `loadChapterFor(pointId)`: The `GrammarChapter` a point belongs to, from the same teaching order.
- `loadContrasts()` / `loadContrastsByFocus()`: The family → lesson/case contrast index (`index/contrasts.json`) and its focus-point-keyed lookup - see the `GrammarPoint.family` note under Core Data Models → Grammar Activity above, and Chapter-driven introduction below for how `lesson.taughtInChapterId` is consumed.
...
- `applyVocabReinforcement(learningQueue, credits, now, settings)`: applies **positive-only** vocab credit for the reinforcement blanks answered correctly in a grammar sentence, feeding each into that word's **production** `SRSEntry` via `SRSService.applyAnswer` (forced result, neutralised latency, scaled by `CONSTANTS.srs.production.reinforcementStrengthRatio`). Filling a blank is English sentence in, Japanese out, which is production's definition; reading asks the opposite question (given the written form, produce its sound) and a blank shows nothing to read, so reading was the one direction of the three this exercise structurally cannot test. Credited at a discount because the conditions are much easier than a production card's: the English sentence, the surrounding Japanese and the bracketing particles narrow the candidates, and kanji is accepted where that card wants the reading. Deliberately **not** split across all three entries - each drives its own schedule, so crediting three would push three due dates out on one scaffolded answer and thin review pressure in two directions it never exercised. A word whose production entry is not yet activated is **seeded first** (`seedProductionEntry`) and credited on top, so it joins the rotation at its designed baseline; a no-op entirely when production quizzes are disabled. `credits` is pre-filtered to correct/minor_error results, and a word not in `learningQueue` is skipped - so grammar practice can lift a word's vocab mastery but never lower it. Returns the same queue reference when nothing changed.
- `getNextCandidates(currentQueue, maxToFind, ignoredIds?)`: walks the dataset's authored teaching order (`GrammarService.loadTeachingOrder()`'s `order`, filtered by `buildTeachabilityFilter` - see below), falling back to a plain JLPT walk (N5 → N1, `GRAMMAR_JLPT_LEVELS`, source order within a level - no `preferredLearningOrder` setting for grammar, since it has no frequency data to sort by, and no kanji filtering either: kanji-awareness applies to which *vocab words* are blanked in a quiz sentence, not to which grammar points can be learned) only when the order file can't be loaded. **Chapter-bounded**: a batch never crosses a chapter boundary - see Chapter-driven introduction below. `countLearnableGrammar`/`hasMoreLearnableGrammar` are NOT chapter-bound (they answer "is there more content at all, anywhere", for the hub's `moreNew` signal), so the two are allowed to disagree in size.
- `getCurrentChapter(currentQueue)`: the `GrammarChapter` containing the next point that would be introduced right now (the earliest point in `order` that isn't active yet and is still teachable). Derived fresh every call, **never stored** - the dataset can re-cut chapters at any time (it already has, for N3-N1), so a persisted "current chapter" pointer would dangle across a re-cut. Returns `null` once nothing is left to introduce, or when the order can't be loaded.
- `buildTeachabilityFilter()`: excludes points the current quiz cannot actually test - a `kind: 'inflection'` point (teaches a derivation, not a fixed string) is only teachable once `conjugations.json` actually carries drill items for it. Not private specifically so `getCurrentChapter`/`getNextCandidates` and the chapter-completion check in `useGrammarOrchestration` (below) share one definition of "teachable" - a chapter containing an untestable point could otherwise never be considered fully introduced.

### Grammar Activity: Chapter-driven introduction and end-of-chapter lessons

The teaching-order chapter (see `GrammarChapter`/`GrammarTeachingOrder` under Core Data Models) is the unit of introduction, not a flat batch off the dataset. **`advanceGrammarQueue` introduces the whole current chapter in one advance**: it sizes its candidate request to `getCurrentChapter().points.length`, and since `GrammarSRSService.getNextCandidates` stops at the current chapter's boundary anyway, that yields every remaining teachable point of the chapter at once (a chapter is authored as a unit meant to be met together, so splitting it across several advances hid that grouping). `CONSTANTS.srs.grammar.newBatchSize` (3) is now only the **fallback ceiling** for the no-chapter path - when the teaching order can't be loaded and `getNextCandidates` walks the flat JLPT order; it is deliberately not shared with vocab's `newVocabBatchSize`. Crossing into the next chapter happens on a later `advanceGrammarQueue()` call, once every teachable point in the current one is active.

**The current chapter is always derived** (`GrammarSRSService.getCurrentChapter`), never stored: the dataset can re-cut chapters at any time (it already has, for N3-N1), and a persisted pointer would dangle across a re-cut. `useGrammarOrchestration` calls it on every `state.progress` change to surface `nextGrammarChapterTitle` (the title of the chapter the next new point would begin), which `MainScreen`'s grammar activity card shows as a "Next chapter: …" line alongside its review/new/retry preview (only when there's new content coming - see Application Pages → Main Screen). `GrammarIntroCard` separately shows a chapter's `title`/`summary` once, on its first point's intro card (pre-existing, from the original teaching-order work - not part of this addition).

**End-of-chapter review step** (`GrammarChapterLessonCard`, `pages/grammar/GrammarChapterLessonCard.tsx`): once every teachable point in a chapter has been introduced, the contrast lessons anchored to it (`lesson.taughtInChapterId === chapterId`) surface once as a consolidated recap, reusing `GrammarContrastCard` per focus point rather than a second set of markup - the same component the per-point intro-time card renders. The two are complementary, not a replacement for each other: the intro-time card is the just-in-time hint (gated per-learner by `selectReadyContrasts`), the chapter-end step is the deliberate review of a now-finished set. A chapter with no anchored lessons shows no step at all - acknowledged silently rather than inventing filler content.

- **`completedChapters: string[]`** (`UserProgress`, see Core Data Models) records which chapters' step has already been shown (or silently acknowledged), so it doesn't re-fire. Purely additive like `grammarQueue`: defaulted to `[]` unconditionally at hydration (`MigrationService.migrateUserProgress`, no version gate) and merged as a pure union in `mergeProgress.ts` (mirroring `mergeGrammarQueues`) - a step shown on one device must not un-show on another.
- **Pure selectors** (`grammarSelectors.ts`): `selectNewlyCompletedChapterIds(chapters, grammarQueue, completedChapters, isTeachable)` - chapters where every TEACHABLE point (mirroring `buildTeachabilityFilter`'s gate, so a chapter stuck on an untestable point isn't stranded incomplete forever) has `introductionAt` set and the chapter isn't already in `completedChapters`. `selectChapterEndFocusIds(contrasts, chapterId)` - the unique `case.focus` ids across every lesson anchored to that chapter, in the order encountered; `GrammarContrastCard` resolves each id's own ready cases on its own, so this only needs to hand it which points to render, not case-level detail.
- **`useGrammarOrchestration`** runs both: an effect (keyed on `state.progress`) recomputes `pendingGrammarChapterLesson` (`{ chapterId, chapterTitle, focusPointIds }` or `null`) - for each newly-completed chapter (in chapter order), a chapter with zero focus ids dispatches `GRAMMAR_CHAPTER_COMPLETE` immediately (no step shown) and moves on; the first one WITH lessons sets the pending state and stops (shown one at a time - dismissing it lets the effect re-run and pick up any more). `grammarActions.dismissGrammarChapterLesson(chapterId)` clears the pending state and dispatches `GRAMMAR_CHAPTER_COMPLETE`, which appends the id to `completedChapters` (no-op, same reference, if already present).
- **`GrammarScreen`** renders `GrammarChapterLessonCard` in place of the loading gate/intro/quiz whenever `pendingGrammarChapterLesson` is set, within the `review`/`learn` states - a deliberate pause on a finished set, not a card the queue serves.

**Families reachable**: `GrammarBrowseScreen`'s family-grouped view (`group === 'family'`) links out to `/grammar/family/:familyId` ("Compare when to use each") on any family group whose `contrasts.json` entry has `lessons.length > 0` - the browse screen's family grouping was already the closest thing to a family index, so it's the natural entry point rather than a new page.
...
- **`quizSelectors.ts`** - `selectNextView(state, hasMoreLearnable, now)` is the **single source of truth** for "what should the quiz screen show right now". It replaces three previously-independent decision points (a queue-level `nextDue` memo, a `computeSessionView` function, and an ad-hoc `currentProgress.introductionAt` check in `VocabQuizScreen`) that could drift out of agreement. Returns `{ queueItem, sessionState, nextReviewAt, shouldShowIntro }`. Also exposes `selectCurrentProgress`, `selectCurrentSentence`, `collectActionableTaskKeys` (every quiz task actionable now, as `TaskKey[]`), `dedupTaskKeysByVocab` (keeps at most one task per vocab, reading > meaning > production - see Session quiz cap below), `capSessionCommit` (truncates a snapshot to `CONSTANTS.srs.sessionQuizCap`, split evenly across quiz types with spillover), `selectSessionStats`, and `selectNextSessionPreview`. `selectSessionStats(state, hasMoreLearnable, now)` returns `{ done, total, retriesPending, waiting, moreNew }` computed against the **raw, unfiltered** `state.session.committed` - the same set `selectNextView` serves from and `selectNextSessionPreview` counts, so the three can never disagree: `total` = committed set size (**fixed** for the session), `done` = committed tasks no longer actionable (answered, deferred, or graduated), `retriesPending` = committed tasks currently awaiting a retry (a wrong answer this session, shown highlighted and appended to the bar denominator), `waiting` = distinct vocab with tasks due *now* that were never committed to this session, `moreNew` = `hasMoreLearnable` (the "+" in "n+ waiting"). This replaced a `done + liveDueReviews` formula whose denominator **shrank on every wrong answer** (a wrong answer pushes the due date ~12h out - leaving the live due count - without incrementing `done`, and the pending retry was never re-counted). `selectNextSessionPreview(state, now)` returns `{ review, new, retries, remaining }`, a preview of what the *next* study session will contain - shown on the Main hub's quiz activity card before the user even clicks in. `review`/`retries` count **quiz cards**, derived by running the session's own pipeline (`collectActionableTaskKeys`, then `dedupTaskKeysByVocab`, then `capSessionCommit`), so the figure on the card is literally what the session will commit to and cannot drift from it. `remaining` is the overflow past dedup+`sessionQuizCap` combined, rendered as "N more after this session". `new` stays a count of **words**, since a queued-but-not-yet-due word produces no task key and how many cards it becomes depends on choices the learner has not made yet. Buckets stay mutually exclusive per word: a retry-flagged word counts once, as a retry.

  This replaced a per-distinct-vocab count whose due-check listed reading and meaning by hand. It was never extended to production, so a queue with only production due reported "all caught up" while the session had a full run of cards waiting - and counting words understated a session that asks up to three cards per word, which matters much more now the cap bounds the sitting.
...
```typescript
{
  ...
  currentProductionSynonyms: ProductionSynonymCandidate[],   // CURRENT production word's near-synonym cluster, resolved before render (issue #71 Part B); [] = no cluster or not a production card
  currentQuizItem: PendingQuizItem | null,
  userAnswer: string,
  feedback: { show, correct, type, message, matchedAnswer, synonymRelation? } | null,
...
**`session` (the committed task set)**: `TaskKey` is `` `${vocabId}:${quizType}` `` (built via the exported `taskKey()` helper). `session.committed` is captured **once** when a study session begins - a snapshot of every quiz task actionable at that moment (`collectActionableTaskKeys`), deduped to at most one task per vocab (`dedupTaskKeysByVocab`, reading > meaning > production) then capped (`capSessionCommit`) - and is extended only by the user's own "Learn" choices (`VOCAB_INTRO_CHOICE` adds the learned word's `reading` task). It never grows from background reviews coming due mid-session.
...
A vocab with more than one direction due at once (e.g. reading and meaning both actionable) contributes **only its highest-priority direction** to `session.committed` - `dedupTaskKeysByVocab` keeps reading over meaning over production, and the excluded direction(s) are simply not part of this session; they surface as `waiting` and are picked up by a later session's own commit once the committed direction is no longer due (see `docs/MODIFICATION_LOG.md`). This replaced a same-session-collision mechanism that lived in `SRSService.applyAnswer` instead: a correct reading answer used to push a due meaning's `dueDate` forward 12h so the two weren't asked back to back. That stagger was asymmetric (production was never staggered) and had a second-order bug once a session could commit both directions for one vocab at all - the stagger silently resolved the committed meaning task's `SessionStats.done` without the user ever answering it, so one answer could advance the progress bar by 2. Deduping at commit time removes the need for the stagger entirely (there's no second committed task left for it to resolve) and covers all three quiz types uniformly, so `applyAnswer` no longer touches any entry's `dueDate` other than the one it was asked to grade. `selectSessionStats` counts `done`/`total`/`retriesPending` against the frozen, already-deduped `session.committed` set as-is - it does not dedup or filter it itself - and reports mid-session arrivals (plus a vocab's deduped-out other directions) separately as "waiting". The lifecycle (`SESSION_START` on entering review/learn, `SESSION_END` on reaching waiting/exhausted) is driven by an effect in `useQuizOrchestration`, which computes the snapshot with `now` so the reducer stays free of `Date.now()`.

Note: this is **not** the old `sessionQueue`/`sessionBuiltAt` subsystem (a prior "frozen session queue" refactor, see historical `[2026-02-28]` log entry, that nothing ever read - deleted as dead code). That one froze whole vocab items; `session.committed` is a task-key set, and retry-on-wrong-answer is still handled entirely by the per-type `needsRetry` flag on `VocabProgress`, not by the session. Note also that `session.committed` **is** now authoritative over what card is served next (see Session quiz cap below) - it was read only by the progress counter before that change.

#### Session quiz cap

`session.committed` bounds what a sitting can serve, not just what its progress counter counts. `selectNextView` passes it to `getNextVocabToStudy` as an `allowed` set, and the commit pipeline - `collectActionableTaskKeys` → `dedupTaskKeysByVocab` → `capSessionCommit` (all `quizSelectors.ts`) - deduplicates to one task per vocab and then truncates to `CONSTANTS.srs.sessionQuizCap` (200) tasks at commit time. Both `selectNextSessionPreview` and the `SESSION_START` snapshot in `useQuizOrchestration` run the exact same three-function pipeline, so the Main hub's preview count, the in-session progress bar's denominator, and what actually gets served can never drift from each other.

Four things about it are load-bearing:

- **Dedup runs before the cap, not after.** `dedupTaskKeysByVocab` keeps at most one task per vocab (reading > meaning > production), so `capSessionCommit`'s even split-by-type operates on the already-deduped pool. Capping first and deduping after would let a vocab's lower-priority direction occupy a cap slot that a dedup pass would otherwise have freed up for a different vocab entirely.
- **The cap is composed per quiz type, not taken as a prefix.** `getNextVocabToStudy` clears every actionable reading before any meaning, so a prefix of the snapshot would commit 200 readings and zero meanings for a user with 400 due, session after session, while the meaning backlog only grew. `capSessionCommit` buckets by type and fills smallest-pool-first, so each type gets an even share and a type with less work than its share spills the surplus to the others rather than cutting the session short. The split is over the types actually present, so it is thirds now that the production quiz exists, and was halves before it. In practice the dedup step skews this: a user with every word's reading due commits almost entirely reading tasks in early sessions, and meaning/production arrive once reading intervals grow past daily - which is the intended "a vocab's other directions naturally never share a session" behavior, not a bug in the split.
- **The committed set is read unfiltered everywhere it's consumed - for serving and for counting.** A meaning-dropping filter (`filterSessionCommit`) once ran at commit time, which made a dropped task unservable: production was unservable in any session where the word's reading was also due, which for a daily reviewer was every session. Moving that filter to run only inside `selectSessionStats` fixed serving but left the progress counter's `total`/`waiting` disagreeing with both what actually gets served and with `selectNextSessionPreview` (unfiltered). Both problems traced back to the same root cause - trying to solve "don't ask both directions of a word in one sitting" by filtering/staggering *after* the commit, rather than deciding it once when the commit is built - which `dedupTaskKeysByVocab` now does instead (see `docs/MODIFICATION_LOG.md`). The committed set is the single deduped, capped actionable set, filtered nowhere downstream of that.
...
`sessionActive` in `useQuizOrchestration` deliberately counts `'session-complete'` as active. Ending the session there would immediately re-satisfy `useSessionLifecycle`'s start condition (work is still due, the route is still `/quiz`) and silently commit a fresh capped set, which would make the cap invisible: the user would just keep being handed cards. Starting another session is the user's explicit choice, via `actions.startNewSession()` (dispatches `SESSION_END`; the lifecycle effect then re-snapshots and re-caps on the next render) from `SessionCompleteScreen`.
...
- `startNewSession()`: End a finished session so the lifecycle effect commits a fresh capped one (the "Start another session" button on `SessionCompleteScreen`)
...
Both activity cards render their preview description via a shared `renderSessionPreviewDescription(preview, nextReviewAt)` helper (`MainScreen.tsx`): `"{review} review · {new} new"`, with `· {retries} retries` appended (in the error color) only when `retries > 0`. When all three counts are 0, it falls back to a caught-up message, showing the next review's ETA when one is known. When `preview.remaining > 0` a second line reads `"{remaining} more after this session"`: only the vocab card can overflow (its session is capped, grammar's is not), and saying so up front is the difference between "this session is the work" and "this session is a slice of it", which changes whether the user stops after it. The vocab card sources its preview from `selectNextSessionPreview(state, now)`; the grammar card from `selectNextGrammarSessionPreview(state, now)` - see State Management for both selectors, and Modification Log `[2026-08-04]`/`[2026-08-05]`. The grammar card also shows a `"Next chapter: {title}"` line beneath the counts, sourced from `nextGrammarChapterTitle` (`useGrammarOrchestration`, derived via `GrammarSRSService.getCurrentChapter` - see Services & Business Logic → Grammar Activity: Chapter-driven introduction), shown only alongside a non-zero `new` count.
...
- **`'review'` / `'learn'`**: `pendingGrammarChapterLesson` (set once a chapter completes and has anchored contrast lessons - see Services & Business Logic → Grammar Activity: Chapter-driven introduction) takes priority over everything else here, rendering `GrammarChapterLessonCard` instead: a deliberate pause on a now-finished chapter, not a card the queue serves. Otherwise: loading gate, then `shouldShowGrammarIntro` (from `selectNextGrammarView`) decides `GrammarIntroCard` vs. a `SessionProgress` header (`stats={grammarSessionStats}`, `history={...grammarSessionHistory mapped to /grammar/:grammarId links}`, `waitingNoun="grammar points"`) plus `GrammarQuizCard`, positioned the same way `VocabQuizScreen` positions its own `SessionProgress` above `VocabQuizCard`/`VocabMeaningQuizCard` (issue #32 follow-up).
...
**`GrammarChapterLessonCard`** (`pages/grammar/GrammarChapterLessonCard.tsx`) - the end-of-chapter review step: a heading naming the completed chapter, one `GrammarContrastCard` per focus point of every lesson anchored to it (`pendingGrammarChapterLesson.focusPointIds`), and a single "Continue" button (`grammarActions.dismissGrammarChapterLesson`). No Learn/Skip choice - there is nothing to introduce here, only to review.
...
### Submit is never gated on filled blanks (`GrammarQuizCard`)

`canSubmitGrammar` checks that a gradable card is loaded and that feedback is not already showing. It does **not** check that every blank has been filled, and that is a deliberate robustness choice rather than a convenience one.

Requiring every input non-empty makes the card **blockable**: any state where an input cannot be filled leaves the learner with no way forward at all, no matter what else is on screen. That has now happened three times, each from a different cause, and the `blankWordSpans` and `GrammarQuizState.example` doc comments record the first two (a multi-token marker split across boxes with no way to know which box wanted which token, and blank indices from one sentence applied to another). The third was the hint button: revealing a blank showed the accepted form as a render-time display value while `grammarAnswers[i]` stayed empty, so revealing the last remaining blank left Submit permanently disabled.

Each individual cause is fixed, but the shared failure mode is the gate itself, so the gate is gone. An empty blank is now a supported answer meaning "I do not know this one": `gradeGrammarAnswers` grades it `'pass'` (the same result a literally typed "pass" gives, -0.15) rather than `'wrong'`, and the accepted form is revealed in the feedback, which is what the learner was reaching for the hint button to get. Auto-advance still requires every blank strictly correct, so a skipped blank always pauses on feedback instead of skipping past the answer.
...
**Grouped by family by default**, not by JLPT level. The page's reason to exist is comparing near-synonyms, and level ordering is already what every other grammar surface presents (the SRS queue, the dictionary's index): family grouping is the view that is not available anywhere else. When grouped by family, a group whose `contrasts.json` entry has at least one lesson gets a "Compare when to use each" link to `/grammar/family/:familyId` in its header - this is the only browse entry point into `GrammarFamilyScreen` (previously reachable only from a contrast card or `GrammarRelatedPointsCard` on a point already introduced).
...
- **Shared related-entry base (`components/RelatedEntriesCard.tsx`)**: `VocabRelationshipsCard`, `GrammarRelatedPointsCard`, and `GrammarVariantsCard` all render through this one presentational component, so the vocab and grammar detail pages' related-entry lists share the exact same Card chrome, row layout (a Japanese `primary`, an inline `secondary` node, an optional right-aligned `meta`, an optional clamped `description`), hover treatment, header link, and cap/expand control. Each caller resolves its own data (and owns the lazy-load + expand state, since only the displayed rows are fetched) and maps it to `RelatedEntry` rows; nothing visual is duplicated per page.
- `GrammarRelatedPointsCard` (`pages/grammar/GrammarRelatedPointsCard.tsx`, issue #42 follow-up, adapted to the named `family` schema in issue #45): the point's near-synonym **family** (`family.relatedPoints`, same core idea at a different formality/nuance) - fetches each related id via `GrammarService.loadGrammarPoint`, renders `family.name` as the header (generic "Related Points" fallback only when `family` is absent) and a clickable list (kana-only title + JLPT chip) to `/grammar/:grammarId` through the shared base, cap/expand at `INITIAL_COUNT = 5`, plus the contrast-gated "Compare when to use each" link. Returns `null` when `family?.relatedPoints` is empty/absent. A realization variant now has no `family`, so this card is empty on a variant page and the point's forms are surfaced by `GrammarVariantsCard` instead.
- `GrammarVariantsCard` (`pages/grammar/GrammarVariantsCard.tsx`): the point's **realization variants** - the SAME point written differently (それでは / それじゃ / じゃ), a distinct relationship from the near-synonym family. Reads `variant-groups.json` via `GrammarService.loadVariantGroups()` keyed on `point.variantOf ?? point.id`, then lists every OTHER group member (fetched for its `jlptLevel`) with a relation chip ("Base form" for the canonical, "Contraction"/"Politeness"/"Particle"/... for the rest) + JLPT chip, under the header "Other forms", through the same shared base. From a variant it links back to its canonical and sibling realizations; from the canonical it links to every realization. Returns `null` when the point has no variant group. This is what made じゃ/それじゃ/それでは reachable from each other on the detail page - previously `GrammarDetailScreen` had no variant awareness at all, so a variant page was a disconnected orphan
...
**`SRSHistoryGraph`** (`components/SRSHistoryGraph.tsx`, renamed from `VocabHistoryGraph.tsx`): generalized to plot an arbitrary list of `{key, label, entry, color}` series against one shared time axis, rather than being hardcoded to vocab's reading/meaning pair - a `GrammarProgress.entry` is shaped exactly like a `VocabProgress.reading`/`meaning` entry for this purpose, so `GrammarDetailScreen` reuses the same component with a single-entry series instead of a second implementation. `VocabDetailScreen` passes reading and meaning, plus production once that entry has been activated; `GrammarDetailScreen` passes one (grammar). The vocab page's Next Review list and mastery rings follow the same rule: production shows a date once activated and the literal text **"Not started"** before that, rather than being hidden. An absent row reads as the feature being broken, which is exactly how the first release was reported.
...
- **Curriculum (`compiled/grammar/index/teaching-order.json`)**: the order points are **introduced** in, replacing `index/jlpt.json`'s alphabetical order for that purpose. Two docs in the dataset submodule cover this: `docs/GRAMMAR_TEACHING_MODEL.md` (hand-written, plain English: what a point, chapter, family, lesson and case each are and how they relate - read it before touching any of this) and `docs/CURRICULUM.md` (generated on every `build:grammar`, the full inventory of every chapter, its points, and the lessons it unlocks). Built by the dataset's `build-curriculum.ts` from an authored N5/N4 spine (`data/curriculum/chapters.json`, 40 hand-sequenced chapters) plus generated N3-N1 chapters. The app reads only the flat `order` array (`GrammarSRSService.getNextCandidates`/`countLearnableGrammar`) and `loadChapterFor`; the chapter arrangement itself is not surfaced in the UI yet. Two rules shape the N3-N1 half, both from the dataset's grammar-curriculum issue:
  - **Themes** (`data/curriculum/themes.json`, 42 authored chapters) replaced an alphabetical dump that put 315 points (42% of the dataset) into 18 buckets of 20 named "Further N2 patterns (3 of 5)". The fallback bucketing still exists so a point added upstream is visibly unthemed rather than silently dropped, and the build names every unthemed point; it is empty today. 145 chapters, median 5 points, max 12 (was 117, max 20).
  - **Absorb vs level-gate** for a family spread across levels: a *pure register ladder* (no `constraint` member) of at most 6 members becomes one chapter at its easiest member's level, so the ladder is taught whole; anything larger or mixed stays level-gated, with the level in the chapter title (`"N2: Concession (Even Though / Although / Despite)"`) so three chapters do not share one name. The size test is over the whole family, not just its register members - splitting a mixed family into an absorbed half and a level-gated half fragments it worse than either rule alone. 12 absorbed, 35 level-gated.
...
- `src/services/srs.service.test.ts` - SRS algorithm unit tests
  ...
  - Production synonym grading tests (issue #71 Part B): `evaluateProductionSynonyms` matching an `interchangeable`/`confusable` candidate by written form or a genuine fuzzy reading typo, exact-only written-form matching (no Levenshtein fallback, mirroring `evaluateProductionAnswer`'s own rule), no match against an unrelated answer or an empty candidate list, and first-match-wins ordering; `applyConfusableSynonymAnswer` leaving memoryStrength/interval/difficulty/dueDate untouched, setting `needsRetry.production` without disturbing another quiz type's flag, still recording `lastReviewedAt`/`totalReviews`, and seeding a fresh entry rather than throwing if somehow unactivated
...
- `src/services/grammarSrs.service.test.ts` - `GrammarSRSService` tests: intro choice (learn/skip), `applyAnswer` (correct/wrong/retry/graduation, mirroring vocab's retry-is-training-only invariant; a reduced `strengthDeltaModifier` earning a smaller-but-positive gain than a full one), `applyVocabReinforcement` (boosts a credited word's reading entry, leaves other words reference-equal, never touches the meaning entry, no-op on empty credits or a word absent from the queue), authored-teaching-order candidate finding/counting (falling back to JLPT order only when the order file can't be loaded) filtered by kind/teachability, and chapter-bounded batching: a batch never crosses a chapter boundary, a chapter with fewer teachable points remaining than the batch size yields only that remainder, the next chapter is only entered on a following call, and `getCurrentChapter` reports the chapter of the next teachable unqueued point (or `null` once nothing is left)
- `src/services/migration.service.test.ts` - Data migration tests
  ...
  - `completedChapters` defaulting to `[]` when absent, and an already-stored list left untouched
- `src/services/migration.roundtrip.test.ts` - Golden round-trip test: a realistic snapshot spanning old/mixed/current-format items pushed through the full migrate→hydrate→serialize→reparse pipeline, asserting zero data loss (no vocab dropped, no history lost, no due date nulled); also covers `completedChapters` defaulting to `[]` when absent from the snapshot and surviving a serialize→reparse round trip when present
- `src/context/quiz/quizReducer.test.ts` - Reducer unit tests (every action, including `RECONCILE_REMOTE`, `SESSION_START`/`SESSION_END`, `VOCAB_INTRO_CHOICE` extending the session's committed task set on "Learn", `LOAD_VOCAB_START`/`LOAD_VOCAB_SUCCESS` resetting/setting `currentProductionCloze`+`productionHintLevel`, `REVEAL_PRODUCTION_HINT`'s level cap + writing the primary reading into `userAnswer` only once level 2 is reached and only with a `currentVocab` present, and (issue #71 Part B) `currentProductionSynonyms` reset on `LOAD_VOCAB_START`/set from the `LOAD_VOCAB_SUCCESS` payload/defaulting to `[]`, plus `feedback.synonymRelation` carried through `SUBMIT_ANSWER`)
...
4. Base Evaluation:
   ...
   - For **Production**: `SRSService.evaluateProductionAnswer()` against readings + written forms (issue #71 Part A - written forms match exactly, readings keep fuzzy matching), then - only if `productionHintLevel >= 2` (the cloze card's hint fully revealed, see Production cloze card) - the result is force-overridden to `minor_error` regardless of what was typed. A `wrong` result (and only then) is additionally checked against the target's precomputed near-synonym cluster via `SRSService.evaluateProductionSynonyms()` (issue #71 Part B - see Production quiz synonym-aware grading): an `interchangeable` match upgrades the result to `minor_error`, a `confusable` match leaves it flagged for `continueToNext` to apply with no strength change instead of the normal penalty. Same grading path for both production cards (gloss-prompt and sentence-cloze, issue #72); only the prompt differs between the two cards themselves
...
**Knowledge points (`utils/knowledge.utils.ts`):**
- `KNOWLEDGE_POINTS_PER_ENTRY`: 200. **One knowledge point is one mastery point**: `entryKnowledgePoints` returns `calculateMasteryPercentage` unchanged, which already spans 0..200 across the MasteryRing's two visual loops. A fully mastered entry is 200 and a word mastered in reading + meaning is 400.
- The scale is arbitrary, so it is set to the one value that removes every conversion. It was 100 per entry, normalising the 0..200 mastery curve by half, which meant the ring advanced 6, the session ticker printed "+6%" and the session total moved by 3: one quantity, three places, two units, and arithmetic the user could not check by eye. Changing the ring to 0..100 instead would unify equally well but would destroy the deliberate two-loop design, so the scale moved rather than the ring.
- Consequence worth knowing: the Stats knowledge curve's absolute numbers are **twice** what they were. The curve's shape, and every comparison drawn from it, is unchanged.
- Production **counts** in the curve alongside reading and meaning, so a word studied in all three directions is worth 600 and a production answer moves the session total and the curve alike. Its no-history entries are the one asymmetry: see Production quiz rollout.
**SRS Limits:**
- `dailyNewLimit`: 20 new vocab per day
- `newVocabBatchSize`: 3 (introduce three at a time for buffered learning) - vocab only, grammar deliberately does not share it
- `grammar.newBatchSize`: 3 - **fallback ceiling for grammar's no-teaching-order path only**. `advanceGrammarQueue` normally introduces the whole current chapter at once (sizing its request to `getCurrentChapter().points.length`); this ceiling applies only when the teaching order can't be loaded and `getNextCandidates` walks the flat JLPT fallback (see Services & Business Logic → Grammar Activity: Chapter-driven introduction)
...
- `production.reinforcementStrengthRatio`: 0.5 (fraction of a full production gain a grammar answer's vocab reinforcement earns; right direction, easier conditions)
