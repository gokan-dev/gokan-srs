...
>
> **Contrast lessons (issue #62)**: `compiled/grammar/index/contrasts.json` (`Record<familyId, { name, clusters: { id, label, memberIds, units: { focus, vs[], situation, guidance }[] }[] }>`) carries authored *situational* disambiguation between confusable family members - the "which one do I reach for, and why not the obvious alternative" that a per-point `usageNote` cannot answer (から and ので both mean "because", but only ので fits an apology). Three distinct concerns: a **chunk** is a confusability-first sub-grouping of a family (target 5-6 members, soft cap; a small family can be one chunk); a **lesson** (unit) is always a subset of one chunk; and **interchangeability** is a separate grading rule (below). Authored in the dataset's `data/raw/grammar/contrasts.json` (AI-drafted, human-reviewed), compiled + strictly validated by `build-grammar.ts`'s pure `compileContrasts` (every `focus`/`vs`/`memberIds` id must be a genuine, non-dropped member of the family it claims, never a `variant`-axis point, and each lesson's `focus`/`vs` must be within its own chunk's `memberIds`). Each unit is **directed** - `focus` is the member met LATER in the teaching order. Consumed via `GrammarService.loadContrasts`/`loadContrastsByFocus`. Surfaced by `GrammarContrastCard` at the `focus` point's introduction, but only once every `vs` sibling is already known (the deferral rule is `utils/grammarContrast.utils.ts`'s pure `selectReadyContrasts`), and revisitably on `GrammarFamilyScreen` (route `/grammar/family/:familyId`, linked from `GrammarRelatedPointsCard`'s "Compare when to use each"). Coverage is now the full set the curriculum calls for: **66 families, 97 chunks, 141 units**, one per chapter-cluster with a real distinction to teach (it was 4 families / 6 units at the starter batch). Chunks are scoped to the confusable set rather than to the family wherever the family grouping was too coarse - `sequence-then` is split into a そして/それから chunk and a じゃ/それでは chunk, because those mean different things and a four-way lesson over them would be incoherent.
>
> Two fields come from the curriculum rather than the authored file. **`chunk.anchorChapterId`** is stamped by `build-curriculum.ts` (the only build step that knows the chapters): the chapter of whichever member the teaching order introduces last, i.e. the earliest point at which every member the lesson names is a real memory. The app does not gate on it - `selectReadyContrasts`'s "every `vs` sibling known" check is the per-learner runtime equivalent - but it is what a chapter-end lesson would key off. The curriculum issue assumed the stronger rule that a chunk may not span chapters **at all**; that rule would delete the から/ので lesson issue #62 opens with (から is introduced in `n5-c16`, ので in `n4-c12`), so what the build enforces instead is the invariant that carries the weight: **a unit's `focus` may never be introduced before one of its `vs` siblings** (a hard error). 3 of 97 chunks span chapters, and the build counts them.
>
> **`family.interchangeable`** is the `axis: 'variant'` members, when a family has two or more. Those can never carry a lesson (a build error if one tries), but staying silent about them is worse than a line of text: a learner meeting the fourth of ten near-identical "whether A or B" forms with no comment concludes a distinction exists and goes hunting for one. `GrammarService.loadInterchangeableByPoint()` keys them by point id; `GrammarContrastCard` renders the note in place of a lesson, deferred by `selectKnownInterchangeableSiblings` until at least one sibling is actually known, and `GrammarFamilyScreen` renders an "Interchangeable" card. A family can carry this and `chunks: []`, so an entry with no chunks is valid rather than missing.
>
...
- **Curriculum (`compiled/grammar/index/teaching-order.json`)**: the order points are **introduced** in, replacing `index/jlpt.json`'s alphabetical order for that purpose. Built by the dataset's `build-curriculum.ts` from an authored N5/N4 spine (`data/curriculum/chapters.json`, 40 hand-sequenced chapters) plus generated N3-N1 chapters. The app reads only the flat `order` array (`GrammarSRSService.getNextCandidates`/`countLearnableGrammar`) and `loadChapterFor`; the chapter arrangement itself is not surfaced in the UI yet. Two rules shape the N3-N1 half, both from the dataset's grammar-curriculum issue:
