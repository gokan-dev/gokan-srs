...
`filterSessionCommit` drops a vocab's `meaning` key whenever its `reading` key is present: answering that reading correctly staggers the meaning's due date forward by 12h (`SRSService.applyAnswer`'s reading to meaning stagger), so counting both credits one answer with clearing two tasks - `done` jumps by 2 instead of 1. It is applied **only by `selectSessionStats`, to its own denominator**, and never to `session.committed` itself. That distinction became load-bearing once the committed set started gating what can be served (see Session quiz cap): a key filtered out of it can never be shown at all, so a *wrong* reading answer (which does not stagger meaning, leaving it genuinely due) would have left that meaning unanswerable for the rest of the session, and production unanswerable in every session where the word's reading was also due. This is what keeps the session-progress counter's denominator **stable**: `selectSessionStats` counts `done`/`total`/`retriesPending` against the frozen set, and reports mid-session arrivals separately as "waiting". The lifecycle (`SESSION_START` on entering review/learn, `SESSION_END` on reaching waiting/exhausted) is driven by an effect in `useQuizOrchestration`, which computes the snapshot with `now` so the reducer stays free of `Date.now()`.
...
- **`filterSessionCommit` is not applied to the committed set at all.** It once was, which was harmless while the set only fed the progress counter. Once the set began gating what can be *served*, filtering it meant a dropped task could never be shown: production was unservable in any session where the word's reading was also due, which for a daily reviewer was every session. The committed set is now the full capped actionable set, and the filter moved to `selectSessionStats`, which is the only place that needs it.
