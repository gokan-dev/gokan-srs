...
The teaching-order chapter (see `GrammarChapter`/`GrammarTeachingOrder` under Core Data Models) is the unit of introduction, not a flat batch off the dataset. `CONSTANTS.srs.grammar.newBatchSize` (3) is a **ceiling**, not a fixed count like vocab's `newVocabBatchSize` (grammar deliberately does not share that constant) - `GrammarSRSService.getNextCandidates` stops at the current chapter's boundary regardless of how much of the ceiling is unfilled, so a chapter with fewer teachable points remaining than the ceiling yields only that remainder. Crossing into the next chapter happens on a later `advanceGrammarQueue()` call, once every teachable point in the current one is active, never mid-batch.
...
- `grammar.newBatchSize`: 3 - grammar's own batch size, a **ceiling** rather than a fixed count: `GrammarSRSService.getNextCandidates` also stops at the current chapter's boundary, so a chapter with fewer teachable points remaining yields only that remainder (see Services & Business Logic → Grammar Activity: Chapter-driven introduction)
