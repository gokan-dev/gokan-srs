...
- **A correct reading answer staggers meaning (+12h) but deliberately NOT production.** The fixed push is stable for meaning because its interval soon diverges from reading's, so it resurfaces on its own. Production does not: it is seeded against reading's own cadence, so for anyone reviewing at roughly the same time each day the two stay due together, the push repeats every session, and production is never once asked. Never asking it is far worse than the redundancy the stagger avoids, and selection serves production last (after every reading and meaning), so it is not asked back-to-back with the same word's reading. Reported from staging after the first release: reading and meaning came up daily while production never appeared at all.
...
- **`quizSelectors.ts`** - `selectNextView(state, hasMoreLearnable, now)` is the **single source of truth** for "what should the quiz screen show right now". It replaces three previously-independent decision points (a queue-level `nextDue` memo, a `computeSessionView` function, and an ad-hoc `currentProgress.introductionAt` check in `VocabQuizScreen`) that could drift out of agreement. Returns `{ queueItem, sessionState, nextReviewAt, shouldShowIntro }`. Also exposes `selectCurrentProgress`, `selectCurrentSentence`, `collectActionableTaskKeys` (every quiz task actionable now, as `TaskKey[]`), `capSessionCommit` (truncates a snapshot to `CONSTANTS.srs.sessionQuizCap`, split evenly across quiz types with spillover - see Session quiz cap below), `selectSessionStats`, and `selectNextSessionPreview`. `selectSessionStats(state, hasMoreLearnable, now)` returns `{ done, total, retriesPending, waiting, moreNew }` computed against the **raw, unfiltered** `state.session.committed` - the same set `selectNextView` serves from and `selectNextSessionPreview` counts, so the three can never disagree: `total` = committed set size (**fixed** for the session), `done` = committed tasks no longer actionable (including one resolved by the reading-to-meaning stagger below without the user directly answering it, not only ones actually answered), `retriesPending` = committed tasks currently awaiting a retry (a wrong answer this session, shown highlighted and appended to the bar denominator), `waiting` = distinct vocab with tasks due *now* that were never committed to this session, `moreNew` = `hasMoreLearnable` (the "+" in "n+ waiting"). This replaced a `done + liveDueReviews` formula whose denominator **shrank on every wrong answer** (a wrong answer pushes the due date ~12h out - leaving the live due count - without incrementing `done`, and the pending retry was never re-counted). `selectNextSessionPreview(state, now)` returns `{ review, new, retries, remaining }`, a preview of what the *next* study session will contain - shown on the Main hub's quiz activity card before the user even clicks in. `review`/`retries` count **quiz cards**, derived by running the session's own pipeline (`collectActionableTaskKeys` then `capSessionCommit`), so the figure on the card is literally what the session will commit to and cannot drift from it. `remaining` is the overflow past `sessionQuizCap`, rendered as "N more after this session". `new` stays a count of **words**, since a queued-but-not-yet-due word produces no task key and how many cards it becomes depends on choices the learner has not made yet. Buckets stay mutually exclusive per word: a retry-flagged word counts once, as a retry.
...
**`session` (the committed task set)**: `TaskKey` is `` `${vocabId}:${quizType}` `` (built via the exported `taskKey()` helper). `session.committed` is captured **once** when a study session begins - a snapshot of every quiz task actionable at that moment (`collectActionableTaskKeys`), unfiltered - and is extended only by the user's own "Learn" choices (`VOCAB_INTRO_CHOICE` adds the learned word's `reading` task). It never grows from background reviews coming due mid-session.
...
A reading-to-meaning stagger (`SRSService.applyAnswer`: answering a reading correctly pushes a due meaning's `dueDate` forward 12h so the two aren't asked back to back in the same sitting) can resolve a committed meaning task without the user ever answering it directly. `selectSessionStats` still counts that as `done` (`done` = committed tasks no longer actionable, regardless of *how* they stopped being actionable) rather than excluding the task from `total` up front. An earlier version excluded it instead, via a `filterSessionCommit` filter applied only inside `selectSessionStats` (never to `session.committed` itself), reasoning that a task very likely to be staggered away shouldn't count as session workload. That desynced the Main hub's preview (`selectNextSessionPreview`, always unfiltered) from the in-session bar - reported from staging as "6 review" on the hub card against "0/3" in the session, with the other 3 mislabeled as "waiting after this session" - and, since staggering is not guaranteed (a *wrong* reading answer never triggers it), could permanently exclude a task from `total` that still went on to be served this session. `total`/`waiting` now read `state.session.committed` directly, matching both what `selectNextView` serves from and what the preview counts, so a committed task always counts toward `total` and is never reported as `waiting`. This is what keeps the session-progress counter's denominator **stable**: `selectSessionStats` counts `done`/`total`/`retriesPending` against the frozen set, and reports mid-session arrivals separately as "waiting". The lifecycle (`SESSION_START` on entering review/learn, `SESSION_END` on reaching waiting/exhausted) is driven by an effect in `useQuizOrchestration`, which computes the snapshot with `now` so the reducer stays free of `Date.now()`.
...
`session.committed` bounds what a sitting can serve, not just what its progress counter counts. `selectNextView` passes it to `getNextVocabToStudy` as an `allowed` set, and `capSessionCommit` (`quizSelectors.ts`) truncates the snapshot to `CONSTANTS.srs.sessionQuizCap` (200) tasks at commit time.
...
- **The committed set is read unfiltered everywhere it's consumed - for serving and for counting.** A meaning-dropping filter (`filterSessionCommit`) once ran at commit time, which made a dropped task unservable: production was unservable in any session where the word's reading was also due, which for a daily reviewer was every session. Moving that filter to run only inside `selectSessionStats` fixed serving but left the progress counter's `total`/`waiting` disagreeing with both what actually gets served and with `selectNextSessionPreview` (unfiltered) - see `selectSessionStats`'s doc comment for the follow-up fix. The committed set is now the single full capped actionable set, filtered nowhere.
...
- `src/context/quiz/quizSelectors.test.ts` - `selectNextView` across all session states + the meaning-disabled edge case, `selectCurrentProgress`, `selectCurrentSentence`, `selectSessionStats` (stable `total`, `done` on de-actioned tasks, the pending-retry regression that no longer shrinks the total, mid-session arrivals counted as `waiting` not total, `moreNew`, a committed meaning task staggered away by its own reading answer still counting toward `total`/`done` rather than being silently dropped - the preview/counter desync regression, and a committed meaning left genuinely due by a wrong reading answer staying out of both `done` and `waiting`), and `selectNextSessionPreview` (mutually exclusive buckets, retries taking precedence over new/review, meaning-disabled ignoring meaning due dates, graduated vocab excluded)
