...
const contrastFor = (focus: string, vs: string[]): GrammarContrastForFocus => ({
    familyId: "causality",
    familyName: "Causality",
    lessonId: "reason-core",
    lessonTitle: "から / ので",
    case: { focus, vs, situation: "s", guidance: "g" },
});

describe("selectReadyContrasts", () => {
    it("keeps a lesson once every vs sibling is known", () => {
        const forFocus = [contrastFor("n4-110", ["n5-073"])];
    ...
    it("defers a lesson while any vs sibling is still unknown", () => {
        const forFocus = [contrastFor("n4-110", ["n5-073", "n5-088"])];
    ...
    it("filters per lesson, keeping the ready ones and dropping the rest", () => {
        const forFocus = [contrastFor("n4-110", ["n5-073"]), contrastFor("n4-110", ["n2-999"])];
        const ready = selectReadyContrasts(forFocus, new Set(["n5-073"]));
        expect(ready).toHaveLength(1);
        expect(ready[0].case.vs).toEqual(["n5-073"]);
    });
